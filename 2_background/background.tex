...
\begin{itemize}
  ...
  \item \texttt{<progress>} specifies the completion progress
        ...
        \texttt{<command>} provides an abstraction layer
        between user interface and commands,
        so that multiple user interface elements can refer
        to the same command. 
...
\begin{itemize}
  ...
  \item \texttt{email} specifies a~control for editing
        ...
        A~regular expression can be used to validate the email. 
  \item \texttt{url} specifies a~control for editing an
        absolute URL given in the element's value.
        A~regular expression can be used to validate the URL. 
  \item \texttt{search} specifies a~one-line plain-text
        edit control for entering one or more textual search terms.
...
\begin{itemize}
  ...
  \item \texttt{XFN} is a~format for representing human
        ...
        to indicate their relationships to other people.
...
\begin{itemize}
  ...
  \item \texttt{itemtype} a~valid URL of a~vocabulary that
        describes the item in question and its properties context.
...
\begin{lstlisting}[caption={
  [Sample code snippet with embedded Microdata mark-up]
  {Sample code snippet with embedded Microdata mark-up
...
In order to encode an RDF graph in XML,
the nodes and predicates have to be represented
in XML terms---element names, attribute names,
element contents, and attribute values.
...
introduced sample graph serialized in Turtle syntax.
...
No information is returned about possible solutions,
...
that build upon those basic pillars.
