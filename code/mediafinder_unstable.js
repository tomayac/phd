    ...
    var services = {
      ...
      */
      ...
      Twitter: function(pendingRequests) {
         ...
         */
        var checkForValidUrl = function checkForValidUrl(url) {
              ...
              for (var i = 0, len = items.length; i < len; i++) {
                ...
                if (item.entities && item.entities.urls) {
                  ...
                  item.entities.urls.forEach(function(url) {
                    if (checkForValidUrl(url.expanded_url)) {
                      tweetUrls.push(url.expanded_url);
                      numberOfUrls++;
                    }
                  });
                }
console.log('Overall pending URLs ' + numberOfUrls)
                var timestamp = Date.parse(item.created_at);
                var publicationDate = getIsoDateString(timestamp);
                var micropost = cleanMicropost(item.text);
                var userProfileUrl = 'http://twitter.com/' + item.from_user;
                tweetUrls.forEach(function(mediaUrl) {
                  var micropostUrl = 'http://twitter.com/' +
                      item.from_user + '/status/' + item.id_str;
                  // yfrog
...
console.log('Yfrog ' + options.url);
                    (function(micropost, userProfileUrl, timestamp, publicationDate) {
                      request.get(options, function(err, result, body) {
                        ...
                        pendingUrls++;
console.log('Pending URLs ' + pendingUrls + '/' + numberOfUrls)
                        if (pendingUrls === numberOfUrls) {
console.log('Sending from Yfrog')
                          collectResults(
                  ...
                  // TwitPic
                  } else if (false /*mediaUrl.indexOf('http://twitpic.com') === 0*/) {
                    var id = mediaUrl.replace('http://twitpic.com/', '');
                    var options = {
                      url: 'http://twitpic.com/' + id + '/full'
                    };
console.log('Twitpic ' + options.url)
                    (function(micropost, userProfileUrl, timestamp, publicationDate) {
                      request.get(options, function(err, res, body) {
...
console.log('Pending URLs ' + pendingUrls + '/' + numberOfUrls)
                          if (pendingUrls === numberOfUrls) {
console.log('Sending from Twitpic')
                            collectResults(
                                results, currentService, pendingRequests);
                            return;
                          }
...
console.log('Img.ly' + options.url)
                    (function(micropost, userProfileUrl, timestamp, publicationDate) {
                      request.get(options, function(err, res, body) {
...
console.log('Pending URLs ' + pendingUrls + '/' + numberOfUrls)
                          if (pendingUrls === numberOfUrls) {
console.log('Sending from Img.ly')
                            collectResults(
                                results, currentService, pendingRequests);
                            return;
                          }
...
console.log('Unsupported ' + mediaUrl)
                    numberOfUrls--;
console.log('Pending URLs ' + pendingUrls + '/' + numberOfUrls)
                    if (pendingUrls === numberOfUrls) {
console.log('Sending from unsupported')
                      collectResults(
                          results, currentService, pendingRequests);
                      return;
                    }
              ...
              if (pendingUrls === numberOfUrls) {
console.log('Sending from results, but no URLs')
                collectResults(
                    results, currentService, pendingRequests);
                return;
              }
...
console.log('Sending from no results')
              collectResults([], currentService, pendingRequests);
              return;
            }
/*
