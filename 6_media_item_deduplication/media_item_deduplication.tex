...
Typically photos are automatically cropped to have a~square aspect ratio,
...
for outerPhoto in photos  
  ...
  for innerPhoto in photos
    ...
    similarTiles = 0
    for tile in distances[outerPhoto][innerPhoto]
      if distances[outerPhoto][innerPhoto] <= SIMILARITY_THRESHOLD
        similarTiles++
      end if   
    end for
...
\subsubsection{Victoria's Secret Fashion Show}
...
The show which was previously taped on November 7, 2012
was aired on December 4, 2012 on CBS with an audience of 9.48 million viewers.
