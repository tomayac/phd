...
This algorithm is fast enough to be applied to hundreds of images
...
Our near-duplicate image clustering algorithm belongs to the family of
tile-wise histogram-based image clustering algorithms.
As an additional semantic feature, the algorithm detects faces
as described in \autoref{sec:face-detection}.
For two images to be clustered,
the following conditions have to be fulfilled.

\begin{enumerate}
  \item The numbers $f_1$ and $f_2$ of detected faces in both images
    have to be the same, ($f_1, f_2 \geq 0$).
    We note that we do \emph{not} recognize faces.
  \item Out of $m$ tiles of an image with $n$ tiles, ($m \leq n$),
...
ROWS = 5
COLS = 5
TILES_THRESHOLD = floor(ROWS * COLS * 0.9)
SIMILARITY_THRESHOLD = 10

histograms = {}
faces = {}
for image in images
  faces[image] = getFaces(image)

  for tile in image
    histograms[image][tile] = getHistogram(tile)
  end for
end for  
    
distances = {}
for outerImage in images
  for innerImage in images
    distances[outerImage][innerImage] = {}
    for tile in histograms[outerImage]
      distances[outerImage][innerImage][tile] =
          abs(histograms[outerImage][tile] - histograms[innerImage][tile])
    end for
  end for
end for
...
for outerImage in images  
  clusters[outerImage] = []  
  for innerImage in images
    if outerImage == innerImage continue
    
    similarTiles = 0
    for tile in distances[outerImage][innerImage]
      if distances[outerImage][innerImage] <= SIMILARITY_THRESHOLD
        similarTiles++
      end if   
    end for
     
    if similarTiles >= TILES_THRESHOLD
      if faces[outerImage] == faces[innerImage]
        clusters[outerImage].push(innerImage)
      end if
    end if
